...
>
    ...
    <xsl:template match="section" mode="url">
        <xsl:text>https://stevenclontz.github.io/exploring-math-technology/</xsl:text>
    ...
    <xsl:template match="task">
        <div>
            <h4>
                ...
                <xsl:number from="//activity|exploration" level="any" count="task" format="A"/>
            </h4>
        </div>
    </xsl:template>
    ...
    <xsl:template match="program">
        <pre><xsl:value-of select="input/text()" /></pre>
    </xsl:template>
    ...
    <xsl:template match="table|figure|definition|example|listing" mode="number">
        <xsl:apply-templates select="./ancestor::section" mode="number"/>.<xsl:number from="//section" level="any" count="table|figure|definition|example|listing"/>
    </xsl:template>

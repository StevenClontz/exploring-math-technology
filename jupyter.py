...
def build():
    ...
    for cindex, chapter in enumerate(book.xpath("//chapter"), start=1):
        if chapter.xpath("@xml:id"):
            jo_directory_team_ch = f"{jo_directory_team}/chapter-{cindex:02d}-{chapter.xpath('@xml:id')[0]}"
            ...
            for sindex, section in enumerate(chapter.xpath("section"), start=1):
                if section.xpath("@xml:id"):
                    ...
                    indv_section_path = f"{jo_directory_indv_ch}/section-{cindex}-{sindex}-{section.xpath('@xml:id')[0]}.ipynb"

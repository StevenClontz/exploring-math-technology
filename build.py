...
PRETEXT = etree.parse("source/main.ptx")
PRETEXT.xinclude()
    ...
    for chapterid, chapter in enumerate(PRETEXT.xpath("//chapter"), start=1):
        for sectionid, section in enumerate(chapter.xpath("section"), start=1):
            ...
            for activity in section.xpath(".//activity"):
                ...
                for intro in activity.xpath("statement|introduction"):
                    for element in intro.xpath("*"):
                        sources.append(element_to_string(element))
                for task in activity.xpath("task"):
                    sources.append(element_to_string(task))
                    for element in task.xpath("*"):
                        sources.append(element_to_string(element))
            ...
            for preview in section.xpath(".//exploration"):
                sources.append(element_to_string(preview,team=False))
                for intro in preview.xpath("statement|introduction"):
                    for element in intro.xpath("*"):
                        sources.append(element_to_string(element,team=False))
                for task in preview.xpath("task"):
                    sources.append(element_to_string(task))
                    for element in task.xpath("*"):
                        sources.append(element_to_string(element))
            for exercise in section.xpath(".//exercises/exercise"):
                if len(exercise.xpath("statement"))>0:
                    ...
                    for intro in exercise.xpath("statement|introduction"):
                        for element in intro.xpath("*"):
                            sources.append(element_to_string(element,team=False))
